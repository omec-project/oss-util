...
namespace EPC
{
   ...
   public:
      ...
      static std::string global_enodeb_id_fqdn( const char *enb, const char *mnc, const char *mcc );
   ...
   public:
      ...
      void get_result( char ***ipv4_hosts )
      {
         int i = 0;
         *ipv4_hosts = (char **)malloc(sizeof (char *) * this->size());

         for (StringVector::const_iterator it = begin();
              it != end();
              ++it)
         {
            (*ipv4_hosts)[i] = (char *)calloc(sizeof(char), 16);
            memcpy((*ipv4_hosts)[i], (*it).c_str(), (*it).length());
            i++;
         }
      }
   ...
   public:
      ...
      void get_result( char *proto )
      {
         memcpy(proto, Utility::getAppProtocol( m_protocol ),
                 strlen(Utility::getAppProtocol( m_protocol )));
      }
   ...
   public:
      ...
      void get_result( char ***protocols )
      {
         int i = 0;
         *protocols = (char **)malloc(sizeof (char *) * this->size());

         for (AppProtocolList::const_iterator it = begin();
              it != end();
              ++it)
         {
            (*protocols)[i] = (char *)calloc(sizeof(char), 16);
            (*it)->get_result(*protocols[i]);
            i++;
         }
      }
   ...
   public:
      ...
      void get_result(dns_query_result_t *result)
      {
         memcpy(result->hostname, m_hostname.c_str(), m_hostname.length());
         result->order = m_order;
         result->port = m_port;
         result->pref = m_preference;
         result->proto_count = m_supported_protocols.size();
         m_supported_protocols.get_result(&result->protocols);
         result->ipv4host_count = m_ipv4_hosts.size();
         m_ipv4_hosts.get_result(&result->ipv4_hosts);
      }
   ...
   public:
      ...
      void get_result(dns_query_result_t *result, uint16_t *res_count)
      {
         int i = 0;
         for (NodeSelectorResultList::const_iterator it = begin();
              it != end();
   ...
   public:
      ...
      NetworkCapability &addDesiredNetworkCapability( const char *nc ) { std::string s(nc); m_desiredNetworkCapabilities.push_back( s ); return m_desiredNetworkCapabilities.back(); }
   ...
   public:
      ...
      void get_result( canonical_result_t *result )
      {
    	  memcpy(result->cano_name1, m_cnn1.getName().c_str(), m_cnn1.getName().length());
    	  memcpy(result->cano_name2, m_cnn2.getName().c_str(), m_cnn2.getName().length());
    	  m_candidate1.get_result(&result->host1_info);
    	  m_candidate2.get_result(&result->host2_info);
    	  result->pair_type = (
    	            m_pairtype == ptColocated ? 1 :
    	            m_pairtype == ptTopologicalDistance ? 2 :
    	            m_pairtype == ptDNSPriority ? 3 : 4);
    	  result->topo_matches = m_topologicalMatches;
      }

      void get_result_fqdn( canonical_result_t *result )
      {
    	  memcpy(result->cano_name2, m_cnn2.getName().c_str(), m_cnn2.getName().length());
    	  m_candidate2.get_result(&result->host2_info);
    	  result->pair_type = (
   ...
   public:
      ...
      ColocatedCandidateList( std::string &fqdn, NodeSelectorResultList &nodelist2);
      ...
      int get_result( canonical_result_t *result)
      {
         int candidate = 1;
         int i = 0;
         for ( ColocatedCandidateList::const_iterator it = begin(); it != end(); ++it )
      ...
      int get_result_fqdn( canonical_result_t *result)
      {
         int candidate = 1;
         int i = 0;
         for ( ColocatedCandidateList::const_iterator it = begin(); it != end(); ++it )
