...
sendRegisterCpCurlReq(std::string strDadmfIpAddr,
	...
	std::string strDadmfUrl = "http://" + strDadmfIpAddr + ":" +
...
int
registerCpOnDadmf(struct in_addr dadmf_addr,
		uint16_t dadmf_port, struct in_addr pfcp_addr,
		struct li_df_config_t *li_df_config, uint16_t *uiCntr) {

	int ret = -1;
	std::string strJsonResponse;
	std::string strDadmfPort = std::to_string(dadmf_port);
	std::string strCpIpAddr = inet_ntoa(pfcp_addr);
	std::string strDadmfIpAddr = inet_ntoa(dadmf_addr);
