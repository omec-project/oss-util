...
std::string Utility::global_enodeb_id_fqdn( const char *enb, const char *mnc, const char *mcc )
{
   std::string s;
...
ColocatedCandidateList::ColocatedCandidateList( std::string &fqdn, NodeSelectorResultList &nodelist2)
   : m_nodelist2( nodelist2 ),
     m_nodelist1( nodelist2 )
{
// create all of the colocated candidate pairs (which also classifies them)
   NodeSelectorResult res;
   ...
   for( NodeSelectorResultList::const_iterator it2 = m_nodelist2.begin();
        ...
        it2++ )
   {
      ColocatedCandidate *cc = new ColocatedCandidate( res, *(*it2) );
      push_back( cc );
   }
